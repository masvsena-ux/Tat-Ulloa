...
<div class="main">
    ...
    <div class="action-container" style="display:flex; margin-left: 95px;">
        <button onclick="window.location.href='{{ route('ventas') }}'" class="btn-submit" style="margin:10px 5px;">
            ← Volver a Tienda
        </button>
        @if(session('rol') == 'U1')
        <button onclick="window.location.href='{{ route('gestion.ventas.inactivas') }}'" class="btn-submit" style="margin:10px 5px;">
            Ver Inactivas
        </button>
        <button onclick="window.location.href='{{ route('gestion.ventas.form') }}'" class="btn-submit" style="margin:10px 5px;">
        Registrar Venta
        </button>
        <button onclick="window.location.href='{{ route('reporte.ventas') }}'" class="btn-submit" style="margin:10px 5px;">
    📊 Ver Reporte
</button>
        @endif
    </div>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td style="display:flex; gap:8px;">
                        <a href="{{ route('gestion.ventas.edit', $v->Id) }}"
                        ...
                        <form action="{{ route('gestion.ventas.inactivar', $v->Id) }}" method="POST"
                              onsubmit="return confirm('¿Inactivar esta venta?')">
                            @csrf
                            <button type="submit"
                ...
                @empty
                <tr>
                    <td colspan="7" style="text-align:center; opacity:0.5;">No hay ventas registradas</td>
                </tr>
                @endforelse
            </tbody>
        </table>
