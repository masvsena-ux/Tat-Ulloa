...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .tienda-main { margin-left: 220px; padding: 32px 40px; min-height: 100vh; }
        ...
        .productos-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(220px, 1fr)); gap: 22px; }
        ...
        .prod-card:hover { transform: translateY(-4px); box-shadow: 0 8px 30px rgba(132,37,147,0.2); }
        .prod-card-img { width: 100%; aspect-ratio: 1/1; object-fit: cover; background: #111; }
        .prod-card-img-placeholder { width: 100%; aspect-ratio: 1/1; background: #111; display: flex; align-items: center; justify-content: center; color: var(--muted); font-size: 0.75rem; }
        .prod-card-body { padding: 14px 16px; display: flex; flex-direction: column; gap: 6px; flex: 1; }
        .prod-card-nombre { font-size: 0.95rem; font-weight: 600; line-height: 1.3; }
        .prod-card-cat { font-size: 0.75rem; color: var(--muted); text-transform: uppercase; letter-spacing: 0.05em; }
        .prod-card-precio { font-size: 1.1rem; font-weight: 700; color: #fff; margin-top: 4px; }
        .prod-card-stock { font-size: 0.75rem; color: var(--muted); }
        ...
        .carrito-header { display: flex; align-items: center; gap: 16px; margin-bottom: 28px; }
        .btn-volver {
        ...
        .carrito-layout { display: grid; grid-template-columns: 1fr 320px; gap: 24px; align-items: start; }
        @media (max-width: 900px) { .carrito-layout { grid-template-columns: 1fr; } }
        ...
        .carrito-item-img { width: 70px; height: 70px; object-fit: cover; border-radius: 8px; flex-shrink: 0; background: #111; }
        ...
        .carrito-resumen h3 { font-size: 1rem; font-weight: 700; margin-bottom: 18px; padding-bottom: 12px; border-bottom: 1px solid var(--border); }
        .resumen-row { display: flex; justify-content: space-between; font-size: 0.88rem; color: var(--muted); margin-bottom: 10px; }
        .resumen-total { display: flex; justify-content: space-between; font-size: 1.05rem; font-weight: 700; color: var(--text); padding-top: 14px; border-top: 1px solid var(--border); margin-top: 8px; }
        ...
        .pago-layout { display: grid; grid-template-columns: 1fr 320px; gap: 24px; align-items: start; }
        @media (max-width: 900px) { .pago-layout { grid-template-columns: 1fr; } }

        .pago-form { background: var(--card); border: 1px solid var(--border); border-radius: var(--radius); padding: 28px; }
        .pago-form h3 { font-size: 1.1rem; font-weight: 700; margin-bottom: 22px; padding-bottom: 12px; border-bottom: 1px solid var(--border); }

        .form-group { margin-bottom: 18px; }
        .form-group label { display: block; font-size: 0.82rem; color: var(--muted); margin-bottom: 7px; text-transform: uppercase; letter-spacing: 0.04em; }
        .form-group input,
        .form-group textarea,
        .form-group select {
        ...
        .metodos-pago { display: grid; grid-template-columns: 1fr 1fr; gap: 10px; }
        .metodo-card {
        ...
        .metodo-card.seleccionado { border-color: var(--accent2); background: rgba(132,37,147,0.12); }
        .metodo-card input[type="radio"] { display: none; }
        .metodo-label { font-size: 0.85rem; font-weight: 600; }
        .metodo-bloqueado { opacity: 0.35; cursor: not-allowed; pointer-events: none; }
        .metodo-pronto { font-size: 0.7rem; color: var(--muted); margin-top: 4px; }
        ...
        .cliente-opcion { padding: 12px 16px; cursor: pointer; border-bottom: 1px solid var(--border); transition: background 0.15s; }
        ...
        .cliente-opcion-vacio { padding: 14px 16px; color: var(--muted); font-size: 0.85rem; text-align: center; }

        .cliente-card {
            display: flex; align-items: center; justify-content: space-between;
            background: rgba(8,51,162,0.12); border: 1px solid rgba(8,51,162,0.35);
        ...
        .cliente-card-quitar { background: none; border: none; color: var(--danger); cursor: pointer; font-size: 1rem; padding: 4px 8px; border-radius: 6px; transition: background 0.2s; }
        .cliente-card-quitar:hover { background: rgba(255,85,85,0.1); }

        .btn-registrar-cliente {
            width: 100%; padding: 10px;
            background: none; border: 1px dashed rgba(132,37,147,0.5);
            border-radius: 8px; color: var(--accent2);
            font-size: 0.85rem; font-weight: 600;
            cursor: pointer; transition: background 0.2s, border-color 0.2s; margin-bottom: 4px;
        }
        .btn-registrar-cliente:hover { background: rgba(132,37,147,0.08); border-color: var(--accent2); }
        ...
        .modal-cerrar { background: none; border: none; color: var(--muted); font-size: 1.1rem; cursor: pointer; padding: 4px 8px; border-radius: 6px; transition: color 0.2s; }
        ...
        .pago-resumen h3 { font-size: 1rem; font-weight: 700; margin-bottom: 16px; padding-bottom: 12px; border-bottom: 1px solid var(--border); }
        .pago-item-mini { display: flex; align-items: center; gap: 10px; margin-bottom: 12px; }
        .pago-item-mini img { width: 44px; height: 44px; object-fit: cover; border-radius: 6px; background: #111; }
        ...
        .exito-wrap { text-align: center; padding: 80px 20px; }
        .exito-icono { font-size: 4rem; margin-bottom: 16px; }
        .exito-wrap h2 { font-size: 1.6rem; font-weight: 700; margin-bottom: 10px; }
        .exito-wrap p { color: var(--muted); margin-bottom: 28px; }
        .btn-seguir { padding: 12px 28px; background: var(--grad); border: none; border-radius: 10px; color: #fff; font-size: 0.95rem; font-weight: 700; cursor: pointer; }
   ...
   style="display:flex; align-items:center; gap:8px; padding:10px 20px; background:linear-gradient(#111,#111) padding-box, linear-gradient(to right,#0833a2,#842593) border-box; border:2px solid transparent; border-radius:999px; color:white; font-size:0.9rem; font-weight:600; text-decoration:none; white-space:nowrap;">
    ...
    <div id="vista-carrito" class="vista">
        <div class="carrito-header">
            <button class="btn-volver" onclick="irATienda()">← Seguir comprando</button>
        ...
        <div class="carrito-layout">
            ...
            <div class="carrito-resumen">
                ...
                <button class="btn-finalizar" id="btn-finalizar" onclick="irAPago()">Finalizar compra →</button>
            </div>
        </div>
    </div>
    ...
    <div id="vista-pago" class="vista">
        <div class="carrito-header">
            <button class="btn-volver" onclick="irACarrito()">← Volver al carrito</button>
        ...
        <div class="pago-layout">
            <div class="pago-form">
                ...
                <button class="btn-confirmar" onclick="confirmarCompra()">Confirmar pedido ✓</button>
            </div>
    ...
    <div id="modal-cliente" class="modal-overlay" style="display:none;" onclick="cerrarModalSiAfuera(event)">
        <div class="modal-box">
            ...
            <div style="display:grid; grid-template-columns:1fr 1fr; gap:12px;">
                <div class="form-group"><label>Primer nombre</label><input type="text" id="nc-p-nombre" placeholder="Laura"></div>
                <div class="form-group"><label>Segundo nombre</label><input type="text" id="nc-s-nombre" placeholder="Sofía"></div>
                <div class="form-group"><label>Primer apellido</label><input type="text" id="nc-p-apellido" placeholder="Martínez"></div>
                <div class="form-group"><label>Segundo apellido</label><input type="text" id="nc-s-apellido" placeholder="Rojas"></div>
            </div>
            <div class="form-group"><label>Correo</label><input type="email" id="nc-correo" placeholder="correo@ejemplo.com"></div>
            <div class="form-group"><label>Teléfono</label><input type="tel" id="nc-telefono" placeholder="3100000000"></div>
            <div class="form-group"><label>Dirección</label><input type="text" id="nc-direccion" placeholder="Calle, número, barrio, ciudad"></div>
            <div class="form-group"><label>Contraseña temporal</label><input type="text" id="nc-pass" placeholder="Mínimo 4 caracteres"></div>
            <button class="btn-confirmar" id="btn-guardar-cliente" onclick="registrarCliente()">Guardar cliente</button>
        </div>
    </div>
...
function agregarAlCarrito(prod) {
    ...
    if (idx >= 0) {
        if (carrito[idx].qty >= prod.stock) { toast('Stock máximo alcanzado', 'err'); return; }
        carrito[idx].qty++;
    } else {
...
function cerrarModalSiAfuera(e) { if (e.target.id === 'modal-cliente') cerrarModalCliente(); }
...
async function registrarCliente() {
    ...
    try {
        const res  = await fetch('{{ route("clientes.store") }}', {
            ...
            body: JSON.stringify({ id, p_nombre: pNombre, s_nombre: sNombre, p_apellido: pApellido,
                s_apellido: sApellido, correo, telefono, direccion, contrasena: pass })
        });
        const data = await res.json();
...
function reiniciar() {
    carrito = []; clienteId = null; metodoSeleccionado = null;
    actualizarBadge();
    limpiarCliente();
